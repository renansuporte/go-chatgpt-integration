...
import (
	...
	"github.com/renansds/fclx/chatservice/internal/domain/entity"
	"github.com/renansds/fclx/chatservice/internal/domain/gateway"
...
type ChatCompletionConfigOutputDTO struct {
	ChatID  string
	UserID  string
	Content string
}
...
type ChatCompletionUseCase struct {
	...
	OpenAiCleint *openai.Client
	Stream       chan ChatCompletionConfigOutputDTO
}

func NewChatCompletionUseCase(chatGateway gateway.ChatGateway, openAiClient *openai.Client, stream chan ChatCompletionConfigOutputDTO) *ChatCompletionUseCase {
	return &ChatCompletionUseCase{
		ChatGateway:  chatGateway,
		OpenAiCleint: openAiClient,
		Stream:       stream,
	}
}

func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionConfigOutputDTO, error) {
	...
	resp, err := uc.OpenAiCleint.CreateChatCompletionStream(
		ctx,
	...
	for {
		...
		r := ChatCompletionConfigOutputDTO{
			ChatID:  chat.ID,
			UserID:  input.UserID,
	...
	return &ChatCompletionConfigOutputDTO{
		ChatID:  chat.ID,
		UserID:  chat.UserID,
		Content: fullResponse.String(),
	}, nil
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error) {
	...
	chat, erro := entity.NewChat(input.UserID, initialMessage, chatConfig)
	if err != nil {
